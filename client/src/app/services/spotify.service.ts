...
export class SpotifyService {
  ...
  searchFor(category:string, resource:string):Promise<ResourceData[]> {
    ...
    var uri = '/search/' +category+'/'+encodeURIComponent(resource);

    return this.sendRequestToExpress(uri).then((data) => {

      if (category == 'artist')
      {
        return data.artists.items.map(x=>new ArtistData(x));
      }
      else if (category == 'track')
      {
        return data.tracks.items.map(x=>new TrackData(x));
      }
      else if (category == 'album')
      {
        return data.albums.items.map(x=>new AlbumData(x));
      }
  ...
  getRelatedArtists(artistId:string):Promise<ArtistData[]> {
    ...
    return this.sendRequestToExpress('/artist-related-artists/'+artistId).then((data) => {
      return data.artists.map(x=>new ArtistData(x));
    });
  }
  ...
  getTopTracksForArtist(artistId:string):Promise<TrackData[]> {
    ...
    return this.sendRequestToExpress('/artist-top-tracks/'+artistId).then((data) => {
      return data.tracks.map(x=>new TrackData(x));
    });
  }
  ...
  getAlbumsForArtist(artistId:string):Promise<AlbumData[]> {
    ...
    return this.sendRequestToExpress('/artist-albums/'+artistId).then((data) => {
      return data.items.map(x=>new AlbumData(x));
    });
  }
  ...
  getTracksForAlbum(albumId:string):Promise<TrackData[]> {
    ...
    return this.sendRequestToExpress('/album-tracks/'+albumId).then((data) => {
      return data.items.map(x=>new TrackData(x));
    });
  }
  ...
  getAudioFeaturesForTrack(trackId:string):Promise<TrackFeature[]> {
    ...
    return this.sendRequestToExpress('/track-audio-features/'+trackId).then((data) => {
      return TrackFeature.FeatureTypes.map(x=> new TrackFeature(x, data[x]));
    });
  }
}
